...
class LineRiderTrack:
    def __init__(self, track: Track):
        ...
        self.version = "6.2"
        self.startPosition = {"x": 0, "y": -1}
        self.riders = [
            {
                "startPosition": {"x": 0, "y": 0},
                "startVelocity": {"x": 0.2, "y": 0},
                "remountable": True,
            }
        ]
        self.lines = []
        ...
        for line in track.lines:
            ...
            if (
                (last_label_date is None)
                or (
                    label_period == "QUARTERS"
                    and self.date_to_quarter(last_label_date)
                    != self.date_to_quarter(line.start_date)
                )
                or (
                    label_period == "MONTHS"
        ...
        if settings.TITLE:
            self.lines.extend(string_to_track(f"${track.ticker}", 0, -200, scale=0.5))
            self.lines.extend(
                string_to_track(
                    f"{track.start_date} TO {track.end_date}", 300, -200, scale=0.2
                )
            )
        if settings.CREDITS:
            self.load_credits()
